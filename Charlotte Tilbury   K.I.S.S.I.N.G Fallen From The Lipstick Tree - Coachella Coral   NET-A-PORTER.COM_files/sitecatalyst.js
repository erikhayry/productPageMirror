...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var s_account="napglobalprod,napprod"
...
s.linkInternalFilters="javascript:,net-a-porter.com,mrporter.com, theoutnet.com"
...
/* External Campaign Tracking */
	...
	s.campaign=s.getValOnce(s.campaign,'s_cm_mmc',0)
	
/* Campaign Pathing */
	s.prop8=s.getAndPersistValue(s.campaign,'campPath',0);
	s.prop8=s.campaign?s.campaign+":"+s.pageName:'';

/* Cross Visit Participation */
	s.eVar6=s.crossVisitParticipation(s.campaign,'s_ev6','30','5','>','',1);
...
/* Keyword Analysis */
	if(s.eVar4) s.eVar4=s.eVar4.toLowerCase()

	var t_search=s.getValOnce(s[s.searchTermVariable],'ev4',0)
	if(t_search=='')
	{	
...
/* Internal Search Type Pulled From Query String */
	if(!s.eVar39) s.eVar39=s.getQueryParam('restype')
	s.eVar39 =s.getValOnce(s.eVar39,'s_restype',0)
	
/* Force Variables to Lowercase */
	if(s.eVar21) s.eVar21=s.eVar21.toLowerCase()
	if(s.eVar22) s.eVar22=s.eVar22.toLowerCase()
	
/* Internal Campaign Tracking */
	s.eVar42=s.getQueryParam('cm_sp')
	s.eVar42=s.getValOnce(s.eVar42,'s_cm_sp',0)

/* Copy Visitor ID into eVar15 */
	s.eVar15="D=s_vi"
...
/* Copy Variables */
	...
	s.prop14="D=g"
        ...
        if(false && "undefined" !== typeof s.eVar24) {
            s.visitorID = s.eVar24;
        }
}
...
s.visitorNamespace="netaporter"
s.trackingServer="metrics.net-a-porter.com"
s.trackingServerSecure="smetrics.net-a-porter.com";



s.setTagContainer("NetAPorter_s");
